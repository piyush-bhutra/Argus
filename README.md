# Argus — Multi-Agent Debate System for Verified Claims

Two LLM agents (Advocate, Skeptic) debate a factual claim across structured rounds. A formal
argumentation engine (Dung's AF grounded extension), an LLM fact-check pass, and a Bayesian
judge combine to produce a truth-probability verdict with a fully auditable trace — instead
of asking one model to just answer.

Course project for **BITE308L (AI theory) + BITE308P (AI Lab)**. See `debate_system_prd.md`
for the full spec, `PROJECT_STATE.md` for current status, and `DEMO.md` for the review script.

## What works today (end-to-end)

`claim → debate orchestrator (LLM) → argument attack graph → grounded extension →
LLM fact-check → Bayesian judge → verdict`, all rendered in the React dashboard.

| Module (syllabus) | Component | File |
|---|---|---|
| M1 Agents | Advocate / Skeptic / Judge with a percept–action loop | `app/services/orchestrator.py` |
| M2 Search (adversarial) | Argument-graph attack/defense evaluation | `app/services/semantics_engine.py` |
| M3 Knowledge Representation | Dung's Abstract Argumentation Framework | `app/services/semantics_engine.py` |
| M4 Reasoning | Fact-check pass over each argument's core assertion | `app/services/fact_checker.py` |
| M5 Uncertainty | Bayesian aggregation of structural + fact-check + confidence signals | `app/services/judge.py` |
| M7 Learning | Isotonic calibration (math built; trained on real labels in Review 2) | `app/services/judge.py` |
| API | FastAPI endpoints + in-memory debate store | `app/api/routes.py`, `app/services/debate_store.py` |
| Pipeline glue | Wires all components together | `app/services/pipeline.py` |
| Frontend | Notebook-style debate page: live transcript, attack graph (grows live, replayable), ruling | `frontend/` |
| Evaluation | FEVER sample; Argus vs a single-LLM baseline — accuracy, ECE, reliability diagrams | `scripts/prepare_fever.py`, `scripts/evaluate.py`, `scripts/reliability_diagram.py` |

**Done since Review 1:** the FEVER sample and the offline evaluation harness — see
[Evaluation](#evaluation).
**Still deferred:** the BM25 retrieval + symbolic triple KB fact-checker, and training the
calibrator.

## Setup

### Backend (FastAPI, Python 3.11)

```bash
py -3.11 -m venv venv
.\venv\Scripts\Activate.ps1
pip install -r requirements.txt
cp .env.example .env      # then edit .env with a real key
```

`.env` (any OpenAI-SDK-compatible provider — currently Google Gemini):

```
LLM_API_KEY=<your Gemini key>       # aistudio.google.com/apikey
LLM_MODEL=gemini-3.5-flash-lite     # newest flash models have a ~20 req/day free cap; use a *-lite
LLM_BASE_URL=https://generativelanguage.googleapis.com/v1beta/openai/
LOG_DIR=./logs
```

Cerebras (`gpt-oss-120b`, base URL `https://api.cerebras.ai/v1`) also works — see
`.env.example`.

Run:

```bash
uvicorn app.main:app --reload
```

API docs at `http://127.0.0.1:8000/docs`.

### Frontend (React 19 + TanStack Start + Tailwind v4)

Needs Node.js 20+.

```bash
cd frontend
npm install
npm run dev
```

Talks to `http://localhost:8000` by default (override with `VITE_API_BASE_URL`). If the
backend is unreachable it falls back to bundled mock data and shows a
**DEMO DATA · BACKEND OFFLINE** chip; if a live debate fails (for example, the LLM quota is
exhausted) it shows **FAILED** instead of substituting fake data. More in
`frontend/README.md`.

The debate page is one scrolling "notebook": a sticky claim bar and scroll-spy rail, the
transcript streaming in turn by turn, the attack graph growing live (and replayable once the
verdict is in, with surviving and defeated arguments marked), then the ruling.

## Cached demo debates

A live 2-round debate is ~5 LLM calls and takes ~50 s. Three debates are also pre-computed
and committed to `data/demo_debates.json`, loaded at startup so these ids resolve instantly
with no API calls (useful if the provider is rate-limited or down):

- `demo-sea-level` — sea level rise has accelerated *(advocate wins)*
- `demo-rust-memory` — Rust eliminates all memory-safety bugs *(skeptic wins)*
- `demo-llm-verify` — LLMs can verify facts without retrieval *(skeptic wins)*

Open them from the landing page, or `GET /debate/demo-sea-level/verdict`.

Regenerate them:

```bash
python -m scripts.build_offline_demos   # deterministic, no LLM
python -m scripts.seed_demos            # from real live debates (needs API budget)
```

## Evaluation

An offline harness scores Argus against FEVER ground truth and against a single-LLM baseline
(one direct "how likely is this claim true?" call). It only *calls* the pipeline — the live API
is unaffected.

```bash
python -m scripts.prepare_fever            # data/fever_sample.json: 25 SUPPORTED / 25 REFUTED (seed 42; --n for more)
python -m scripts.evaluate --limit 10      # class-balanced smoke run first
python -m scripts.evaluate                 # full sample, ~6 LLM calls per claim
python -m scripts.reliability_diagram      # data/reliability_argus.png, data/reliability_baseline.png
```

Each score is cached the moment it lands — per claim *and* per half (Argus / baseline), stamped
with the config that produced it. A run killed by the daily quota resumes where it stopped, and
a config change re-scores only the half it affects. Code is not part of the cache key: run with
`--refresh` after changing the debate, fact-check, or judge code.

**Current results** (balanced n=50, 2026-09-10, `gemini-3.5-flash-lite`, no calibrator yet):

| System | n scored | Accuracy @0.5 | ECE | mean P(true) |
|---|---|---|---|---|
| Argus | 49 | 0.633 | 0.283 | 0.233 |
| Single-LLM baseline | 46 | 0.848 | 0.150 | 0.502 |

The baseline currently wins. Argus is systematically skeptical — its mean P(true) is 0.23 on a
set that is 50% true — and that is the number the fact-checker and calibrator work has to move.
Caveats (n≈50, 4 claims with a missing half) are in `PROJECT_STATE.md` §4.

## Tests

```bash
python -m pytest -v
```

52 tests, 0 skipped — semantics, judge/calibration, LLM client, orchestrator, fact checker,
pipeline, an end-to-end API smoke test, and the evaluation harness (sample balancing, metrics,
cache resume and corruption fail-safes). Every LLM call is mocked; no network or API key needed.
